...
impl Component for Jumbotron {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let model_link = ctx.props().link.clone();
        html! {
            <div class="jumbotron">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="row">
                            <div class="col-sm-6 smallpad">
                                <button type="button" id="run" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Run(1_000))}>{ "Create 1,000 rows" }</button>
                            </div>
                            <div class="col-sm-6 smallpad">
                                <button type="button" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Run(10_000))} id="runlots">{ "Create 10,000 rows" }</button>
                            </div>
                            <div class="col-sm-6 smallpad">
                                <button type="button" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Add(1_000))} id="add">{ "Append 1,000 rows" }</button>
                            </div>
                            <div class="col-sm-6 smallpad">
                                <button type="button" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Update(10))} id="update">{ "Update every 10th row" }</button>
                            </div>
                            <div class="col-sm-6 smallpad">
                                <button type="button" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Clear)} id="clear">{ "Clear" }</button>
                            </div>
                            <div class="col-sm-6 smallpad">
                                <button type="button" class="btn btn-primary btn-block" onclick={model_link.callback(|_| Msg::Swap)} id="swaprows">{ "Swap Rows" }</button>
                            </div>
                        </div>
                    </div>
                </div>
